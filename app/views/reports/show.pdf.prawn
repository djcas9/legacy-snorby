...
pdf.text "#{@report.rtype.capitalize} Security Report", :size => 15, :style => :bold, :align => :center
...
unless @report.events.blank?
  ...
  if @report.from_time.blank? || @report.to_time.blank?
    pdf.image open(Gchart.line(:axis_labels => ["#{@report.events.first.timestamp.strftime('%D - %I:%M %p')}|#{Time.now.strftime('%D - %I:%M %p')}"], :axis_with_labels => ['x','y'], :line_color => @_line_color, :data => @_line_data, :size => '500x230')), :position => :center
  else
    pdf.image open(Gchart.line(:axis_labels => ["#{Time.parse(@report.from_time).strftime('%D - %I:%M %p')}|#{Time.parse(@report.to_time).strftime('%D - %I:%M %p')}"], :axis_with_labels => ['x','y'], :line_color => @_line_color, :data => @_line_data, :size => '500x230')), :position => :center
  end
  pdf.move_down(5)
end
unless @report.events.blank?
