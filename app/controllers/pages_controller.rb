class PagesController < ApplicationController
  ...
  def dashboard
    @g_event_severity ||= open_flash_chart_object(400,200, pie_event_severity_graph_url(:high => @high, :medium => @medium, :low => @low)) unless @all.blank?
    @g_category_information ||= open_flash_chart_object(400,200, bar_event_severity_graph_url(:high => @high, :medium => @medium, :low => @low, :all => @all.size)) unless @all.blank?
    @events ||= @all
