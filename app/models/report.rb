class Report < ActiveRecord::Base
  ...
  def find_events
    Event.find(:all, :include => [:sensor, :iphdr, {:sig => :sig_class }], :conditions => ['timestamp >= ? AND timestamp <= ?', DateTime.parse(from_time), DateTime.parse(to_time)], :order => 'timestamp DESC', :group => 'signature')
